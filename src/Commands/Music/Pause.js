...
module.exports = class extends Command {
	...
	async run(message) {
		...
		if (player.paused) return message.channel.send(new MessageEmbed().setDescription(`The song is already paused.`).setColor('BLACK'));

		player.pause(true);
		return message.channel.send(new MessageEmbed().setDescription(`Paused the song.`).setColor('BLACK'));
	}
